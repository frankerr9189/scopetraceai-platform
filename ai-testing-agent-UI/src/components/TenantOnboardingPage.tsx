...
export function TenantOnboardingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${TEST_PLAN_API_BASE_URL}/api/v1/onboarding/tenant`, {
        method: 'POST',
        headers: {
      ...
      const data: TenantCreateResponse = await response.json()

      // Store tenant info temporarily for next step
      sessionStorage.setItem('onboarding_tenant_id', data.tenant_id)
      sessionStorage.setItem('onboarding_tenant_slug', data.tenant_slug)
      sessionStorage.setItem('onboarding_tenant_name', data.tenant_name)

      // Navigate to admin user creation
      navigate(`/onboarding/admin?tenant_id=${data.tenant_id}`, { replace: true })
    } catch (err) {
