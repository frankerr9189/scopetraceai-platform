...
def check_entitlement(db: Session, tenant_id: str) -> Tuple[bool, Optional[str], Optional[str], Optional[int]]:
    """
    Check if tenant is allowed to run a Jira writeback execute operation.
    ...
    Returns:
        ...
        - remaining: Number of trial writeback runs remaining (None if not Trial or Active)
    ...
    try:
        ...
        # The db session is already from the testing agent backend, so we can import directly
        # But we need to ensure the path is set up correctly
        ...
        # Clear cached modules to force re-import from correct location
        modules_to_clear = [k for k in sys.modules.keys() if k.startswith(('models',))]
        for mod in modules_to_clear:
            if 'models' in mod and mod != 'models':
                del sys.modules[mod]
...
def consume_trial_run(db: Session, tenant_id: str) -> None:
    """
    Consume one trial writeback run for the tenant.
    Decrements trial_writeback_runs_remaining by 1.
    ...
    try:
        ...
        if subscription_status == "Trial":
            if trial_writeback > 0:
                new_writeback = trial_writeback - 1
                tenant.trial_writeback_runs_remaining = new_writeback
                ...
                if trial_requirements == 0 and trial_testplan == 0 and new_writeback == 0:
                    ...
                    logger.info(
                        ...
                        f"({trial_writeback} -> {new_writeback}). "
                        f"Remaining: requirements={trial_requirements}, testplan={trial_testplan}, writeback={new_writeback}"
                    )
                
                db.commit()
            else:
                logger.warning(f"Tenant {tenant_id}: Attempted to consume trial run but remaining={trial_writeback}")
        else:
