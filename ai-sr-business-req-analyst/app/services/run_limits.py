"""
Run limit enforcement service for BA Requirements Agent.
...
Same logic as ai-testing-agent/backend/services/run_limits.py.
"""
import logging
...
def check_and_increment_run_usage(
    ...
    Args:
        ...
        current_period_start: Stripe period start (optional, can be string or datetime)
        current_period_end: Stripe period end (optional, can be string or datetime)
    ...
    try:
        ...
        # Convert period strings to datetime if needed
        period_start_dt = None
        period_end_dt = None
        
        if current_period_start:
            if isinstance(current_period_start, str):
                period_start_dt = datetime.fromisoformat(current_period_start.replace('Z', '+00:00'))
            else:
        ...
        period_key = compute_current_period_key(period_start_dt, period_end_dt)
        
        # Compute period boundaries (for storage)
        period_start, period_end = compute_usage_period(period_start_dt, period_end_dt)
