#!/usr/bin/env python3
"""
Dev script to verify BA agent entitlements fix.

This script verifies that:
1. check_entitlement() checks trial_requirements_runs_remaining (not testplan)
2. consume_trial_run() decrements trial_requirements_runs_remaining (not testplan)

Run this after any changes to app/services/entitlements.py to prevent regression.
...
try:
    ...
    print("Make sure you're running from the ai-sr-business-req-analyst directory")
    sys.exit(1)


def test_consume_decrements_requirements():
    """Test that consume_trial_run decrements requirements counter, not testplan."""
    print("Test 1: Verifying consume_trial_run decrements requirements counter...")
    ...
    if mock_tenant.trial_requirements_runs_remaining != 2:
        print(f"  ❌ FAIL: trial_requirements_runs_remaining should be 2, got {mock_tenant.trial_requirements_runs_remaining}")
        return False
    ...
    if mock_tenant.trial_writeback_runs_remaining != 3:
        print(f"  ❌ FAIL: trial_writeback_runs_remaining should remain 3, got {mock_tenant.trial_writeback_runs_remaining}")
        return False
    
    print("  ✅ PASS: Requirements counter decremented correctly, testplan/writeback untouched")
    return True


def test_check_entitlement_checks_requirements():
    """Test that check_entitlement checks requirements counter, not testplan."""
    print("Test 2: Verifying check_entitlement checks requirements counter...")
    ...
    if remaining != 2:
        print(f"  ❌ FAIL: check_entitlement should return 2 (requirements), got {remaining}")
        return False
    
    if not allowed:
        print(f"  ❌ FAIL: Should be allowed when requirements remaining > 0, got allowed={allowed}")
        return False
    ...
    print("  ✅ PASS: check_entitlement correctly checks requirements counter")
    return True
...
def test_paywall_flip():
    ...
    if mock_tenant.trial_requirements_runs_remaining != 0:
        print(f"  ❌ FAIL: Requirements should be 0, got {mock_tenant.trial_requirements_runs_remaining}")
        return False
...
def main():
    ...
    print("=" * 60)
    print("BA Agent Entitlements Fix Verification")
    print("=" * 60)
    print()
    
    results = []
    results.append(test_consume_decrements_requirements())
    print()
    results.append(test_check_entitlement_checks_requirements())
    print()
    ...
    if all(results):
        ...
        print("BA agent correctly decrements trial_requirements_runs_remaining")
        return 0
    else:
        print("❌ SOME TESTS FAILED")
        print("BA agent may be decrementing the wrong counter!")
        return 1
